...
from libalgorithm import ALGORITHMS
...
PORT_LIST_ROUTER = [
  ...
  153, # Simple Gateway Monitoring Protocol (SGMP), remote inspection and alteration of gateway management information
  ...
  ####
  524, # NetWare Core Protocol (NCP) is used for acceess to primary NetWare server resources, Time Synchronization, etc.
  ...
  655, # Tinc VPN daemon
  657, # IBM RMC (Remote monitoring and Control) protocol, used by System p5 AIX Integrated Virtualization Manager (IVM)
  ...
  701, # (TCP) Link Management Protocol (LMP), a protocol that runs between a pair of nodes and is used to manage traffic engineering (TE) links
  ...
  ####
  3478, # STUN, a protocol for NAT traversal; TURN, a protocol for NAT traversal (extension to STUN); STUN Behavior Discovery. See also port 5349.
  ...
  ####
  5349, # STUN over TLS/DTLS, a protocol for NAT traversal; TURN over TLS/DTLS, a protocol for NAT traversal; STUN Behavior Discovery over TLS. See also port 3478.
  ...
  19999, # Distributed Network Protocol—Secure (DNP—Secure), a secure version of the protocol used in SCADA systems between communicating RTU's and IED's
