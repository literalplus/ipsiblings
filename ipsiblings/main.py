...
def main():
  ap = argparse.ArgumentParser(description = 'IP Siblings Toolset\n\nThe argument of -c/-t option (combined with -s option) can be used with alexa\ntop list file if resolution is required.\n[Any other file formatted in that way can be used.]', add_help = False, formatter_class = argparse.RawDescriptionHelpFormatter)
  ...
  mutualgrp.add_argument('-c', '--candidates', action = 'store', help = 'parse candidates from csv file or top list (-s)', default = None, nargs = '?', const = 'None')
  mutualgrp.add_argument('-t', '--trace-targets', action = 'store', help = 'trace target hosts from csv file or top list (-s)', default = None, nargs = '?', const = 'None')
  # -l also works for already constructed TraceSets in combination with Alexa/Cisco Top list
  mutualgrp.add_argument('-l', '--load', action = 'store_true', help = 'load previously saved trace sets from base directory', default = False)
  mutualgrp.add_argument('--alexa-toplist', action = 'store', dest = 'alexa_toplist_dir', help = 'loads the alexa top list from the internet and saves it to the given directory or current working directory', default = None, nargs = '?', const = 'cwd')
  ...
  opt_grp.add_argument('-d', '--directory', action = 'store', help = 'base directory to store and load trace sets', default = None)
  opt_grp.add_argument('-i', '--ignore-file', action = 'store', help = 'nodes to ignore are listed in this file', default = None)
  opt_grp.add_argument('-r', '--run-harvest', action = 'store_true', help = 'perform harvesting for candidate IPs', default = False)
  opt_grp.add_argument('-s', '--resolved', action = 'store_true', help = 'construct candidates or trace targets from resolved (alexa top) list (use with -c/-t for operation mode)', default = False)
  opt_grp.add_argument('-f', '--resolved-file', action = 'store', help = 'csv file holding resolved (alexa) domains and IPs', default = None)
  opt_grp.add_argument('-o', '--download-alexa', action = 'store_true', help = 'allows downloading alexa top list from the internet', default = False)
  opt_grp.add_argument('--router-ports', action = 'store_true', help = 'use the comprehensive port list for non-server devices', default = False)
  opt_grp.add_argument('--server-ports', action = 'store_true', help = 'use the much smaller port list for servers', default = False)
  opt_grp.add_argument('--from', action = 'store', type = int, dest = 'start_index', help = 'restricts candidates/targets to a start index', default = None)
  opt_grp.add_argument('--to', action = 'store', type = int, dest = 'end_index', help = 'restricts candidates/targets to an end index (excluded)', default = None)
  opt_grp.add_argument('--low-runtime', action = 'store_true', help = 'use only few timestamps for evaluation', default = False)
  opt_grp.add_argument('--print', action = 'store_true', help = 'print charts to pdf file', default = False)
  opt_grp.add_argument('--resultfile', action = 'store', help = 'write evaluation results to file', default = None, nargs = '?', const = const.RESULT_FILE_NAME)
  opt_grp.add_argument('--no-evaluation', action = 'store_true', help = 'do not perform any calculations/evaluations on sibling candidates', default = False)
  opt_grp.add_argument('--cdn-file', action = 'store', help = 'load CDN networks for IP filtering', default = None)
  opt_grp.add_argument('--write-pairs', action = 'store', help = 'write constructed IP pairs to file', default = None, nargs = '?', const = const.IP_PAIRS_FILE_NAME)
  ...
  ARGS_candidates_available = bool(ARGS_candidates_csv_file) # evaluate to true if no additional argument for -c/-t is given
  ARGS_targets_available = bool(ARGS_target_csv_file)        # this allows us to distinguish whether candidate or target tasks should be performed
  ...
  if const.IFACE_IP4_ADDRESS and const.IFACE_IP6_ADDRESS:
    log.debug('Identified IP addresses [{0}]: {1} / {2}'.format(nic, const.IFACE_IP4_ADDRESS, const.IFACE_IP6_ADDRESS))
  ...
  if ARGS_load_tracesets:
    ...
    TRACE_SETS = libtrace.load_trace_sets(ARGS_base_dir, base_dir_silent, v4bl_re = v4bl_re, v6bl_re = v6bl_re, iface = nic)
    ...
    if TRACE_SETS and (ARGS_start_index or ARGS_end_index): # slice the data set as requested
      ...
      if ARGS_start_index >= ARGS_end_index:
        log.error('Start index can not be higher/equal than end index (start/end: {0} / {1})'.format(ARGS_start_index, ARGS_end_index))
        return -6
      elif ARGS_start_index > trace_sets_length:
        log.error('Start index is higher than the number of TraceSets available (start index / data size: {0} / {1}'.format(ARGS_start_index, trace_sets_length))
        return -6

      keys = list(TRACE_SETS.keys())[ARGS_start_index : ARGS_end_index]
      TRACE_SETS = { key: TRACE_SETS[key] for key in keys }

      log.info('Reduced loaded TraceSets from size [{0}] to [{1}] (indices from [{2}] to [{3}])'.format(trace_sets_length, len(TRACE_SETS), ARGS_start_index, ARGS_end_index))
  ...
  ##########
    ...
    if ARGS_resolved:
      ...
      ipdata = const.ALEXA.construct_targets(one_per_domain = False) # gives ~250k targets for 145k resolved hosts of Alexa Top List
      if not ipdata:
        if ARGS_resolved_file:
    ...
    if ARGS_write_pairs:
       nr_records = libtools.write_constructed_pairs(pathlib.Path(ARGS_base_dir) / ARGS_write_pairs, ipdata, include_domain = include_domain)
       log.info('Wrote [{0}] IP candidate pairs to [{1}]'.format(nr_records, str(pathlib.Path(ARGS_base_dir) / ARGS_write_pairs)))
    ...
    if ARGS_start_index or ARGS_end_index: # slice if requested
      ...
      if ARGS_start_index >= ARGS_end_index:
        ...
        log.error('Start index is higher than the number of targets available (start index / data size: {0} / {1}'.format(ARGS_start_index, length))
        return -6

      ipdata = ipdata[ARGS_start_index : ARGS_end_index]

      log.info('Reduced targets from size [{0}] to [{1}] (indices from [{2}] to [{3}])'.format(length, len(ipdata), ARGS_start_index, ARGS_end_index))
    ...
    try:
      ...
      for n, target in enumerate(ipdata, start = 1):
        ...
        if ARGS_cdn_file:
          if const.CDNFILTER.is_cdn(ip4, ip6):
            CDN_FILTERED[(ip4, ip6)] = domains
            if domains:
              info_str = '({0} of {1}) Filtered target (CDN) {2} / {3} [{4}]'.format(n, ipdata_len, ip4, ip6, domains)
            else:
        ...
        while nr_current_traces < const.NR_TRACES_PER_TRACE_SET:
          ...
          ip4tracert, ip6tracert = libtraceroute.CPTraceroute((ip4, ip6), iface = nic, algorithm = 'traceroute', timeout = 2).traceroute(result_timeout = 3)
          ...
          try:
            trace = libtrace.Trace().init(ip4, ip6, ip4tracert, ip6tracert, v4bl_re = v4bl_re, v6bl_re = v6bl_re, iface = nic)
          except ValueError:
            trace = None
          ...
          if not trace or trace.id() in trace_set.get_traces():
            ...
            if trace:
              log.debug('Trace {0} (with target {1} / {2}) already in current trace set! [{3}. retry]'.format(trace.id(), ip4, ip6, no_new_trace_counter))
            else:
              log.debug('No trace data available for target ({0} / {1})! [{2}. retry]'.format(ip4, ip6, no_new_trace_counter))

            if no_new_trace_counter >= const.MAX_TRIES_FOR_NEW_TRACE:
              break
            continue

          nodes4, nodes6 = trace.get_global_valid_IPs(apply_ignore_regex = bool(ARGS_ip_ignore_file)) # only apply regex if ignore file was given
      ...
      if const.WRITE_INACTIVE_TRACE_SET:
        ...
        if ts_written > 0 or ts_silent_written > 0:
          log.info('Active TraceSets written: {0} / Inactive TraceSets written: {1}'.format(ts_written, ts_silent_written))
      else:
        if ts_written > 0:
  ...
  ##########
    ...
    if ARGS_resolved:
      ...
      # keep in mind that slicing does not yield deterministic results if one_per_domain is True
      CANDIDATE_PAIRS = const.ALEXA.construct_candidates(one_per_domain = False) # gives ~ 500k candidates for 145k resolved hosts of Alexa Top List
      ...
      ports_available, ts_data_available, tcp_opts_available, CANDIDATE_PAIRS = libts.load_candidate_pairs(ARGS_candidates_csv_file, v4bl_re = v4bl_re, v6bl_re = v6bl_re, include_domain = True)
      if not CANDIDATE_PAIRS:
    ...
    if ARGS_start_index or ARGS_end_index: # slice data
      ...
      if ARGS_start_index >= ARGS_end_index:
        ...
        log.error('Start index is higher than the number of CandidatePairs available (start index / data size: {0} / {1}'.format(ARGS_start_index, length))
        return -6

      keys = list(CANDIDATE_PAIRS.keys())[ARGS_start_index : ARGS_end_index]
      CANDIDATE_PAIRS = { key: CANDIDATE_PAIRS[key] for key in keys }

      log.info('Reduced candidates from size [{0}] to [{1}] (indices from [{2}] to [{3}])'.format(length, len(CANDIDATE_PAIRS), ARGS_start_index, ARGS_end_index))
    ...
    try:
      ...
      if not ports_available:
        ...
        while not cpscan.finished():
          ...
          cpscan.process_results(ip_cp_lut, timeout = 1.5) # 1.5 seconds seems to be the optimum for debug output
      ...
      if not ports_available:
        nr_candidates_written, nr_data_records_written = libts.write_candidate_pairs(CANDIDATE_PAIRS, ARGS_base_dir, only_active_nodes = True, write_candidates = True, write_ts_data = False, write_tcp_opts_data = True, include_domain = True)
  ...
  if ARGS_perform_harvesting and not ARGS_candidates_available:
    ...
    if not any([ ts.has_timestamp_data() for ts in TRACE_SETS.values() ]):
      ...
      try:
        harvester = libts.TraceSetHarvester(TRACE_SETS, runtime = const.HARVESTING_RUNTIME, interval = const.HARVESTING_INTERVAL, iface = nic)
    ...
    if not ts_data_available:
      ...
      try:
        harvester = libts.CandidateHarvester(CANDIDATE_PAIRS, runtime = const.HARVESTING_RUNTIME, interval = const.HARVESTING_INTERVAL, iface = nic)
        harvester.start()
        ...
        nr_candidates_written, nr_data_records_written = libts.write_candidate_pairs(CANDIDATE_PAIRS, ARGS_base_dir, write_candidates = False, write_ts_data = True, write_tcp_opts_data = False, include_domain = True)
  ...
  if (ARGS_candidates_available and ts_data_available) or any([ ts.has_timestamp_data() for ts in TRACE_SETS.values() ]):
    candidates = None
    if ARGS_candidates_available:
  ...
  if not ARGS_no_ssh_keyscan:
    ...
    sshkeyscan = keyscan.Keyscan(candidates, directory = ARGS_base_dir, timeout = None, key_file_name = const.SSH_KEYS_FILENAME, agent_file_name = const.SSH_AGENTS_FILENAME, keyscan_command = const.SSH_KEYSCAN_COMMAND)
  ...
  index = random.randrange(0, len(candidates)) # 0, 22, 177[very high raw tcp ts diff] (1k hz), 95 (250 hz) to test, 6 (ranodmized ts)
