...
def construct_evaluated_features_to_train(gt_dir, lrt=True, feature_keys=['raw_timestamp_diff'], split_nlnog_ripe=True,
                                          batch_size=5000, timestamp_limit=None):
    print('{0} - Start loading true sibling data ...'.format(now()))

    valid_sibs = list(
        load_data(gt_dir, 'c', lrt=lrt, include_domain=True, limit_nr_timestamps=timestamp_limit).values())

    print(
        '{0} - Finished loading true sibling data ... Started [ splitting and ] constructing false siblings ...'.format(
            now()))
    ...
    gc.collect()
    print(
        '{0} - Finished constructing true sibling features ... Started preparation of batches for void sibling feature construction ...'.format(
            now()))
...
def construct_model(gt_siblings, model_class, *args, lrt=True, feature_keys=['raw_timestamp_diff'],
                    missing_values_handler=None, split_nlnog_ripe=True, batch_size=5000, model_file_name=None,
                    model_fimportance_fname=None, **kwargs):
    """
    ...
    **kwargs                any keyword arguments which should be given to model_class
    ...
    if isinstance(gt_siblings, str) or isinstance(gt_siblings, pathlib.PurePath):
        ...
        X, y = construct_evaluated_features_to_train(gt_siblings, lrt=lrt, feature_keys=feature_keys,
                                                     split_nlnog_ripe=split_nlnog_ripe, batch_size=batch_size)
    ...
    if model_file_name:
        ...
        if model_fimportance_fname:
            ...
            xgb.plot_importance(model, ax=axes, height=0.5, color='lightgreen', edgecolor='black',
                                title='Feature Importance', xlabel='F-score', ylabel='Features',
                                importance_type='weight', grid=False, show_values=True)
...
def evaluate_router_traces(base_dir, model, lrt, feature_keys):
    ...
    for dir in dirs:
        ...
        nr_edge_routers, nr_intermediate_routers, nr_siblings_edge_routers, nr_siblings_intermediate_routers = evaluate_routers(
            dir, model, lrt=lrt, feature_keys=feature_keys)
    ...
    print('Intermediate siblings/non-siblings: {0} / {1}'.format(intermediate_siblings,
                                                                 intermediate_routers - intermediate_siblings))
...
def evaluate_routers(directory, model, lrt=False, feature_keys=['raw_timestamp_diff'], return_objects=False,
                     edge_prefix_v4='/22', edge_prefix_v6='/32'):
    """
    ...
    try:
        while True:
            ...
            if sibcand.is_sibling == True:  # is set in evaluate() which was called previously -> check raw_timestamp_diff < 0.305211037
                ...
                is_sibling = model.predict(
                    pd.DataFrame([sibcand.get_features(key_list=feature_keys, substitute_none=np.nan)],
                                 columns=feature_keys))[0]  # should only have 1 result
    ...
    if return_objects:
        return (nr_edge_routers, nr_intermediate_routers, nr_siblings_edge_routers, nr_siblings_intermediate_routers,
                siblings_edge, non_siblings_edge, siblings_intermediate, non_siblings_intermediate)
    else:
        return (nr_edge_routers, nr_intermediate_routers, nr_siblings_edge_routers, nr_siblings_intermediate_routers)
...
def construct_void_siblings(siblings, lrt):
    ...
    for sibkey, validsib in siblings.items():
        for k in keys:
            ...
            if lrt:
                s = libsiblings.LowRTSiblingCandidate(validsib.ip4, currentsib.ip6, validsib.port4, currentsib.port6,
                                                      validsib.ip4_ts, currentsib.ip6_ts, validsib.ip4_tcpopts,
                                                      currentsib.ip6_tcpopts, ssh_available=ssh_available,
                                                      ssh_keys=ssh_keys)
            else:
                s = libsiblings.SiblingCandidate(validsib.ip4, currentsib.ip6, validsib.port4, currentsib.port6,
                                                 validsib.ip4_ts, currentsib.ip6_ts, validsib.ip4_tcpopts,
                                                 currentsib.ip6_tcpopts, ssh_available=ssh_available, ssh_keys=ssh_keys)
...
def stats(y_true, y_pred, raw=False):
    ...
    return (sensitivity, specificity, precision, negative_predictive_value, false_negative_rate, false_positive_rate,
            false_discovery_rate, false_omission_rate, accuracy, f1_score, mcc, rates)
...
def scheitle_evaluation(data_directory, lrt=True, test_size=0.33, feature_keys=None, mcc_weights=False, batch_size=5000,
                        split_nlnog_ripe=False, print_tree=True):
    if not feature_keys:
    ...
    S_train, S_test, y_train, y_test = ms.train_test_split(list(valid_sibs.values()),
                                                           [1 for _ in range(len(valid_sibs))], test_size=test_size,
                                                           random_state=42)
    ...
    print(
        '{0} - Constructed features of true siblings ... Now constructing false sibling features for training ...'.format(
            now()))
    ...
    print(
        '{0} - Constructed features of false siblings for training ... Now constructing false sibling features for testing ...'.format(
            now()))
    ...
    print(
        '{0} - Constructed features of false siblings for testing ... Now preparing data and weights and performing training ...'.format(
            now()))
    ...
    print('{0} - Before dropna() - training samples: {1:6} - test samples: {2:6}'.format(now(), xtrain_before,
                                                                                         xtest_before))
    ...
    print(
        '{0} -  After dropna() - training samples: {1:6} - test samples: {2:6} - train difference: {3:6} - test difference: {4:6}'.format(
            now(), xtrain_after, xtest_after, xtrain_before - xtrain_after, xtest_before - xtest_after))
    ...
    if print_tree and type(clf) == tree.DecisionTreeClassifier:
        ...
        if lrt:
            tree.export_graphviz(clf,
                                 out_file='/root/thesis/data/_DATA/eval/lrt_cmp_scheitle_CART_{0}.dot'.format(cur_time),
                                 feature_names=feature_keys, filled=True, rounded=True)
            with open('/root/thesis/data/_DATA/xgb_eval/lrt_cmp_scheitle_CART_{0}.pickle'.format(cur_time),
                      mode='wb') as outfile:
                pickle.dump(clf, outfile)
        else:
            tree.export_graphviz(clf,
                                 out_file='/root/thesis/data/_DATA/eval/frt_cmp_scheitle_CART_{0}.dot'.format(cur_time),
                                 feature_names=feature_keys, filled=True, rounded=True)
            with open('/root/thesis/data/_DATA/xgb_eval/frt_cmp_scheitle_CART_{0}.pickle'.format(cur_time),
                      mode='wb') as outfile:
                pickle.dump(clf, outfile)
    ...
    (sensitivity_train, specificity_train, precision_train, negative_predictive_value_train, false_negative_rate_train,
     false_positive_rate_train, false_discovery_rate_train, false_omission_rate_train, accuracy_train, f1_score_train,
     mcc_train, rates_train) = stats(y_train['y_train'], y_pred_train, raw=True)
    (sensitivity_test, specificity_test, precision_test, negative_predictive_value_test, false_negative_rate_test,
     false_positive_rate_test, false_discovery_rate_test, false_omission_rate_test, accuracy_test, f1_score_test,
     mcc_test, rates_test) = stats(y_test['y_test'], y_pred_test, raw=True)
    ...
    print('test   sens, spec, npv, fnr: {0}, {1}, {2}, {3}'.format(sensitivity_test, specificity_test,
                                                                   negative_predictive_value_test,
                                                                   false_negative_rate_test))
    print('test fpr, fdr, for, acc, f1: {0}, {1}, {2}, {3}, {4}'.format(false_positive_rate_test,
                                                                        false_discovery_rate_test,
                                                                        false_omission_rate_test, accuracy_test,
                                                                        f1_score_test))
    print('train  mcc (sklearn, stats): {0}, {1}'.format(
        metrics.matthews_corrcoef(y_train, y_pred_train, sample_weight=train_weights), mcc_train))
    print('test   mcc (sklearn, stats): {0}, {1}'.format(
        metrics.matthews_corrcoef(y_test, y_pred_test, sample_weight=test_weights), mcc_test))
...
def xgb_evaluation(data_directory, output_directory=None, own_model=False, lrt=True, test_size=0.33, feature_keys=None,
                   mcc_weights=False, batch_size=5000, split_nlnog_ripe=False, print_tree=True,
                   limit_nr_timestamps=None):
    if not feature_keys:
        feature_keys = ['raw_timestamp_diff']
    ...
    if own_model:
        ...
        params = {'eta': 0.1, 'n_estimators': 800, 'max_depth': 4, 'min_child_weight': 6, 'gamma': 0.05,
                  'subsample': 0.6, 'colsample_bytree': 0.6, 'nthread': 4}

        if low_runtime:  # tune parameters ?!
            clf = xgb.XGBClassifier(
                **params)  # max_depth = 10, random_state = 42) # objective = 'binary:logistic', booster = 'gbtree', max_depth = 10, n_estimators = 64, learning_rate = 0.01, subsample = 0.5, random_state = 42, missing = np.nan)
        else:
            clf = xgb.XGBClassifier(
                **params)  # max_depth = 10, random_state = 42) # objective = 'binary:logistic', booster = 'gbtree', max_depth = 10, n_estimators = 64, learning_rate = 0.01, subsample = 0.5, random_state = 42, missing = np.nan)
    ...
    if print_tree:
        ...
        pdf_features = matplotlib.backends.backend_pdf.PdfPages(
            output_directory / 'xgb_{0}_{1}{2}.features.pdf'.format(identifier, cur_time, add_ts_nr))
        pdf_trees = matplotlib.backends.backend_pdf.PdfPages(
            output_directory / 'xgb_{0}_{1}{2}.trees.pdf'.format(identifier, cur_time, add_ts_nr))
        clf.save_model(str(output_directory / 'xgb_{0}_{1}{2}.model'.format(identifier, cur_time, add_ts_nr)))
        with open(output_directory / 'xgb_{0}_{1}{2}.pickle'.format(identifier, cur_time, add_ts_nr),
                  mode='wb') as outfile:
            pickle.dump(clf, outfile)
        ...
        fig = plt.figure(dpi=256)
        axes = plt.subplot(111)
        xgb.plot_importance(clf, ax=axes, height=0.5, color='lightgreen', edgecolor='black', title='Feature Importance',
                            xlabel='F-score', ylabel='Features', importance_type='weight', grid=False, show_values=True)
        pdf_features.savefig(fig)
        plt.close(fig)
        pdf_features.close()
    ...
    (sensitivity_train, specificity_train, precision_train, negative_predictive_value_train, false_negative_rate_train,
     false_positive_rate_train, false_discovery_rate_train, false_omission_rate_train, accuracy_train, f1_score_train,
     mcc_train, rates_train) = stats(y_train, y_pred_train, raw=True)
    (sensitivity_test, specificity_test, precision_test, negative_predictive_value_test, false_negative_rate_test,
     false_positive_rate_test, false_discovery_rate_test, false_omission_rate_test, accuracy_test, f1_score_test,
     mcc_test, rates_test) = stats(y_test, y_pred_test, raw=True)
    ...
    if best_score and best_iteration and best_ntree_limit:
        print('train        early stopping: {0} best_score, {1} best_iteration, {2} best_ntree_limit'.format(best_score,
                                                                                                             best_iteration,
                                                                                                             best_ntree_limit))
...
def print_cart_feature_importance(modelpath, features, feature_plot_path):
    ...
    fig = plt.figure(dpi=256)
    axes = plt.subplot(111)
    xgb.plot_importance(feature_dict, ax=axes, height=0.5, color='lightgreen', edgecolor='black',
...
if __name__ == '__main__':
    ...
    # DO NOT CHANGE ORDER
    feature_keys = {}
    feature_keys['full'] = ['hz_diff', 'hz_rsqrdiff', 'raw_timestamp_diff', 'alphadiff', 'rsqrdiff', 'dynrange_diff',
                            'dynrange_avg', 'dynrange_diff_rel', 'spl_diff', 'spl_diff_scaled']
    feature_keys['all'] = ['hz_diff', 'hz_rsqrdiff', 'raw_timestamp_diff', 'alphadiff', 'rsqrdiff', 'dynrange_diff',
                           'dynrange_avg', 'dynrange_diff_rel']
    feature_keys['four'] = ['raw_timestamp_diff', 'hz_diff', 'dynrange_diff', 'dynrange_diff_rel']
    feature_keys['one'] = ['raw_timestamp_diff']
    feature_keys['no_raw'] = ['hz_diff', 'hz_rsqrdiff', 'alphadiff', 'rsqrdiff', 'dynrange_diff', 'dynrange_avg',
                              'dynrange_diff_rel']
    # models:
    ...
    if '/GT/' in str(training_data_dir).upper() or (
            '/ALEXA/' in str(training_data_dir).upper() and identifier == 'FRT'):
        no_subfolders = True
    else:
        no_subfolders = False
    ...
    for ts in test_sizes:
        ...
        print()
        print('{0} - Started'.format(now()))
        xgb_evaluation(training_data_dir, output_directory=None, own_model=True, lrt=low_runtime, test_size=ts,
                       feature_keys=feature_keys[fkey], mcc_weights=False, batch_size=5000, split_nlnog_ripe=True,
                       print_tree=False, limit_nr_timestamps=limit_timestamps)
