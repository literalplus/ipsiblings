...
class TopDownTracerouteAlgorithm(Algorithm):
    ...
    def init(
            self,
            algorithm_params={}
    ):  # target_ip, timeout = 2, iface = 'en0', sport = 179, dport = 80, filter = None, retry = 0, multi = False, store_unanswered = False):
        """
    ...
    def run(self):
        ...
        if self.ipversion == const.IP4:
            ...
            base_pkt = scapy.Ether() / scapy.IP(src=own_ip, dst=str(self.target_ip)) / scapy.TCP(sport=self.sport,
                                                                                                 dport=self.dport,
                                                                                                 flags='S',
                                                                                                 options=[('WScale', 0),
                                                                                                          ('Timestamp',
            ...
            base_pkt = scapy.Ether() / scapy.IPv6(src=own_ip, dst=str(self.target_ip)) / scapy.TCP(sport=self.sport,
                                                                                                   dport=self.dport,
                                                                                                   flags='S', options=[
                    ('WScale', 0), ('Timestamp', (ts, 0))])
        ...
        a, u = scapy.srp(to_send, timeout=self.timeout, verbose=0, filter=self.filter, retry=self.retry,
                         multi=self.multi_responses, store_unanswered=self.store_unanswered)
        ...
        trace = dict(
            sorted(icmp_responses.items()))  # { ttl: icmp_responses[ttl] for ttl in sorted(icmp_responses.keys()) }
