...
class CPTraceroute(object):
    ...
    def __init__(
        ...
        libtools.validate(((type(target) is tuple and len(target) == 2) or (type(target) is str)),
                          'Wrong type for [target], should be a string (hostname) or a tuple of length 2 (IPv4, IPv6) [was \'{0}\']'.format(
                              type(target)))
        libtools.validate((type(algorithm) is str and algorithm in const.ALGORITHMS_AVAILABLE),
                          'Illegal input, [algorithm] should be string and one of \'{0}\', [was \'{1}\']'.format(
                              str(const.ALGORITHMS_AVAILABLE), algorithm))
        libtools.validate(protocol in ('udp', 'tcp', 'icmp'),
                          'Illegal input, [protocol] should be one of (\'udp\', \'tcp\', \'icmp\') [was \'{0}\']'.format(
                              protocol))
        ...
        libtools.validate((type(timeout) is int and timeout <= 3600 and timeout >= 1),
                          'Illegal input, [timeout] should be int and between 1 and 3600 seconds [was \'{0}\']'.format(
                              timeout))
        ...
        if type(target) is str:
            ...
            if not addresses:
                raise TracerouteException(
                    'Hostname \'{0}\' does not have an IPv4 or IPv6 address. In doubt provide a tuple \'(IPv4, IPv6)\'.'.format(
                        target))
        ...
        self.tr4 = Traceroute(self.ipv4, iface, algorithm, protocol, srcport, dstport, min_ttl, max_ttl, timeout,
                              detect_broken_nat, const.IP_VERSION_4)
        self.tr6 = Traceroute(self.ipv6, iface, algorithm, protocol, srcport, dstport, min_ttl, max_ttl, timeout,
                              detect_broken_nat, const.IP_VERSION_6)

        if self.hostname:
            log.debug('CPTraceroute created for host \'{0}\' (IPv4: {1}, IPv6: {2})'.format(self.hostname, self.ipv4,
                                                                                            self.ipv6))
        else:
    ...
    def traceroute(self, result_timeout=None):
        ...
        with multiprocessing.Pool(processes=2) as pool:
            ...
            try:
                ...
                log.error(
                    'Timeout during IPv4 traceroute! Increase existing timeout ({0} seconds) or check trace manually!'.format(
                        result_timeout))
                self.v4trace = None
            except Exception as e:
                log.error('Exception: {0} - {1}'.format(type(e).__name__,
                                                        e))  # prevents KeyError in traceroute algorithm if target IP is not in result set
                self.v4trace = None
            ...
            try:
                ...
                log.error(
                    'Timeout during IPv6 traceroute! Increase existing timeout ({0} seconds) or check trace manually!'.format(
                        result_timeout))
                self.v6trace = None
            except Exception as e:
                log.error('Exception: {0} - {1}'.format(type(e).__name__,
            ...
            if self.v4length != self.v6length:
                log.debug(
                    'Traces differ in length, v4 trace [{0}], v6 trace [{1}]'.format(self.v4length, self.v6length))
            else:
