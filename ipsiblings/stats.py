...
def get_port_stats_base(basedir):
  tracesets = list(libts.load_trace_sets(basedir).values()) # only objects in list
  v4ports = set()
  v6ports = set()
  v4map = {} # { port: count }
  v6map = {} # { port: count }
...
def is_montonically_increasing(list_of_timestamp_tuples, strict = False):
  arr = numpy.array([x[0] for x in sorted(list_of_timestamp_tuples, key = lambda x: x[1])]) # sort by arrival time, get only received ts
  if strict:
...
def get_number_of_randomized_nodes_traces(dir, initial_ts_threshold = 10000):
  candidates = list(libsiblings.construct_trace_candidates(libtrace.load_trace_sets(dir), low_runtime = True).values())
  randomized = 0
...
def get_path_node_stats(dir, return_sets = False):
  tracesets = list(libtrace.load_trace_sets(dir).values())
