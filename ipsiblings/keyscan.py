...
class Keyscan(object):
  ...
  def __init__(self, candidates, directory = None, timeout = 600, key_file_name = const.SSH_KEYS_FILENAME, agent_file_name = const.SSH_AGENTS_FILENAME, keyscan_command = const.SSH_KEYSCAN_COMMAND):
  ...
  def _scan(self, in_data, out_data, err_data):
    proc = subprocess.Popen(shlex.split(self.keyscan_command), stdin = subprocess.PIPE, stdout = subprocess.PIPE, stderr = subprocess.PIPE, universal_newlines = True, cwd = self.directory)
