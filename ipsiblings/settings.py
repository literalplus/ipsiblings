...
def dependency_error():
    ...
    if python_version_error:
        log.error("Please update your Python 3 installation to version {0}.{1}!".format(const.PYTHON_VERSION_MAJOR,
                                                                                        const.PYTHON_VERSION_MINOR))
...
class Settings(object):
    ...
    def __init__(self, backup_to_file=True):
        ...
        if self.uname.system.lower().startswith('linux'):
    ...
    def _backup_to_file(self):
        ...
        if self.saved_settings:
            ...
            try:
                ...
                log.error('Exception while backing up [{0}] and value [{1}]: {2}'.format(current_option, current_value,
                                                                                         str(e)))
                return False
        else:
